...
class PostTypeRegistry(object):
	...
	def register(self, model):
		if model.__name__ in self._registry:
			raise AlreadyRegistered('Post type %s is already registered.' % model.__name__)

		self._registry[model.__name__] = model

	def unregister(self, model):
		if model.__name__ not in self._registry:
			raise NotRegistered('Post type %s is not registered.' % model.__name__)
	...
	def content_types_lookup(self):
		...
		"""
		qs = [Q(model__iexact=name) for (name, model) in self.items()]
